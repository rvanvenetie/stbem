...
class InitialOperator:
    ...
    def linform_vector(self, elems=None, cache_dir=None, use_mp=False):
        ...
        if not use_mp:
            ...
            globals()['__M0'] = self
            vec = np.array(mp.Pool(mp.cpu_count()).map(MP_M0_val, range(N)))
...
if __name__ == "__main__":
    ...
    init_mesh = initial_mesh.UnitSquare()
    u0 = lambda y: np.sin(y[0]) * y[1]
    x = np.array([[0.5], [0.5]])
    #t = 0.01
    #val_exact = 0.237147179846976417036758053345
    t = 0.001
    val_exact = 0.239473176349241907505021205474

    #t = 0.001
    #x = np.array([[1], [1]])
    #val_exact = 0.198013791933202147920593597921


    def f(y):
        xy = x - y
        xy_sqr = np.sum(xy**2, axis=0)
        return 1. / (4 * np.pi * t) * np.exp(-xy_sqr / (4 * t)) * u0(y)

    for i in range(6):
        for quad_order in range(5, 150, 2):
            quad_scheme = ProductScheme2D(gauss_quadrature_scheme(quad_order))
            val = 0
            for elem in init_mesh.leaf_elements:
                val += quad_scheme.integrate(f, float(elem.vertices[0].x),
                                             float(elem.vertices[2].x),
                                             float(elem.vertices[0].y),
                                             float(elem.vertices[2].y))
            ...
            print(
                'i = {}, N = {}, C = {}'.format(
                    i, quad_order,
                    len(init_mesh.leaf_elements) * len(quad_scheme.weights)),
                (abs((val - val_exact) / val_exact)))
        init_mesh.uniform_refine()
